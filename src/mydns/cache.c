...
zone_cache_find(TASK *t, uint32_t zone, char *origin, dns_qtype_t type,
  ...
  CACHE			*C = NULL;			/* Which cache to use when inserting */
    // Add by Moriarti <mor.moriarti@gmail.com> at 2016-05-03 18:10:12 GMT+2.
    static char		client_addr[16];
    //
  *errflag = 0;
  ...
  if (!name)
    ...
    // Add by Moriarti <mor.moriarti@gmail.com> at 2016-05-03 18:12:28 GMT+2.
  ...
  if (ZoneCache) {
	...
	if ((n->namelen == namelen) && (n->zone == zone) && (n->type == type)) {
	  ...
	  if (!memcmp(n->name, name, namelen)) {
	    // Modify by Moriarti <mor.moriarti@gmail.com> at 2016-05-03 18:21:38 GMT+2.
	//    if ( (!memcmp(n->name, name, namelen))
	//	    && (!memcmp(n->client_addr, client_addr, sizeof(client_addr)))
	//    {
	    //
	    /* Is the node expired? */
	    if (n->expire && (current_time > n->expire)) {
	...
	if (!memcmp(n->name, name, namelen)) {
	// Modify by Moriarti <mor.moriarti@gmail.com> at 2016-05-03 18:53:27 GMT+2.
	//if ( (!memcmp(n->name, name, namelen))
	//	    && (!memcmp(n->client_addr, client_addr, sizeof(client_addr)))
	//{
	//
...
//    if (mydns_rr_load_active(sql, &rr, zone, type, name, origin,client_addr) != 0) {
  ...
  //
  //strncpy(n->client_addr, client_addr, sizeof(n->client_addr)-1);
  //
  if (type == DNS_QTYPE_SOA) {
    if (C == ZoneCache)
