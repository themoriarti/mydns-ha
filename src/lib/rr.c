...
// Add by Moriarti <mor.moriarti#gmail.com>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <regex.h>
...
// GeoDNS
char *geo_get(json_object *geo, char *clientAddr)
{
    const char *returnedCountry;
    GeoIP *gi;
    json_object *countryObj;
    gi = GeoIP_open("/data/GeoIP.dat", GEOIP_MEMORY_CACHE);
    returnedCountry = GeoIP_country_code_by_addr(gi, clientAddr);
    countryObj = json_object_object_get(geo, returnedCountry);
    char *countryIP = json_object_get_string(countryObj);
    //size_t forward_length = strlen(countryIP);
    if (countryIP != '\0') {
	return countryIP;
    }else{
	json_object *defObj = json_object_object_get(geo, "default");
	char *defIP = json_object_get_string(defObj);
	GeoIP_delete(gi);
	return defIP;
    }
}
...
int mydns_rr_load_all_filtered(SQL *sqlConn, MYDNS_RR **rptr, uint32_t zone,
			       dns_qtype_t type,
			       const char *name, const char *origin, const char *filter, char *clientAddr) {
